...
data_name = 'NEW_yale_sp_TRAIN-010-030-050_TEST-no-noise';
clstNum = 15;
maxit = 200;
% -------------------------------------------------------------------------
diary(now_time);
...
load(['.\dataset\\Yale+B\',data_name,'.mat'])
...
fprintf('#####  [ train Linf ]  #####\n');
model = "Linf";
[trainCore_Linf, An_Linf, Out_Linf, time_Linf] = extractTrainFeaTime(trainTensor,model, opt);
fprintf('#####  [ train L2 ]  #####\n');
model = "L2";
[trainCore_L2, An_L2, Out_L2, time_L2] = extractTrainFeaTime(trainTensor,model, opt);
fprintf('#####  [ train L1 ]  #####\n');
model = "L1";
[trainCore_L1, An_L1, Out_L1, time_L1] = extractTrainFeaTime(trainTensor,model, opt);
fprintf('-------------------------------------------------------------\n');
opt.C = C_test;
fprintf('#####  [ test Linf ]  #####\n');
model = "Linf";
...
%% happy ending
yourFolder = ['./CacheData/time_test/'];
if ~exist(yourFolder, 'dir')
   mkdir(yourFolder);
end
save(['./CacheData/time_test/yale'])
diary off
